...
umxSummaryDoC <- function(model, digits = 2, comparison = NULL, std = TRUE, showRg = FALSE, CIs = TRUE, report = c("markdown", "html"), file = getOption("umx_auto_plot"), returnStd = FALSE, zero.print = ".", ...) {
  ...
  if (typeof(model) == "list") { # call self recursively
    ...
    specifics <- data.frame(
      row.names = paste0("Specific ", c("a", "c", "e")), stringsAsFactors = FALSE,
      rbind(
        diag(as),
        diag(cs),
        diag(es)
    ...
    if (showRg) {
      ...
      rA <- tryCatch(solve(sqrt(nVarIden * A)) %*% A %*% solve(sqrt(nVarIden * A)), error = function(err) {
        return(NAmatrix)
      }) # genetic correlations
      rC <- tryCatch(solve(sqrt(nVarIden * C)) %*% C %*% solve(sqrt(nVarIden * C)), error = function(err) {
        return(NAmatrix)
      }) # C correlations
      rE <- tryCatch(solve(sqrt(nVarIden * E)) %*% E %*% solve(sqrt(nVarIden * E)), error = function(err) {
        return(NAmatrix)
      }) # E correlations
