...
xmuGSEM_write_common_factor <- function(S_LD) {
	varNames = colnames(anthro$S_LD)
	k = length(varNames)
	# m1 = umxRAM("commonFactorModel",data = varNames,
	# 	umxPath(var= varNames), umxPath(var= "F1", fixedAt=1),
	# 	umxPath(from = "F1", to = varNames)
	# )
	
	# F1 =~ NA*BMI + WHR + CO + Waist + Hip + Height + IHC + BL + BW
	# F1 ~~ 1*F1
	for (i in 1) {
		linestart = paste0("F1", " =~ NA*", varNames[i])  
		if ((k-i) > 0) {
			linemid = ""
			for (j in (i+1):k) {
				linemid = paste0(linemid, " + ", varNames[j])
			}
		} else {
			linemid = ""
		}
	}
	Model1 = paste0(linestart, linemid, " \n ", "F1 ~~ 1*F1", " \n ")
	return(Model1)
}

# Code to write null model for calculation of CFI
xmuGSEM_write_independence_model <- function(S_LD, label1 = "V", label2 = "VF") {
	varNames = colnames(anthro$S_LD)
	k = length(varNames)
	# m1 = umxRAM("independenceModel", umxPath(var= varNames),data = varNames)
	varStr = "" # variances
	for (p in 1:k) {
		linestart3 = paste0(varNames[p], " ~~ ", varNames[p])
		varStr = paste0(varStr, linestart3, " \n ")
	} 

	Model2 = "" # latents fixed @ 1 VF1 =~ 1*BMI
	for (p in 1:k) {
    	linestart2 = paste0(label2, p, " =~ 1*", varNames[p])
		Model2 = paste0(Model2, linestart2, " \n ")
	}

	Modelsat = "" # setting all covariances to zero BMI ~~ 0*WHR
	for (i in 1:(k-1)) {
		linestartc = paste(varNames[i], " ~~ 0*", varNames[i+1],  sep = "")
		if (k-i >= 2) { 
			linemidc = ""
			for (j in (i+2):k) {
				linemidc = paste(linemidc, " + 0*", varNames[j])
      	  	}
    	} else {
			linemidc = ""
		}
	...
	ModelsatF = "" # all latents independent VF1 ~~ 0*VF2
	for (i in 1:(k-1)) {
		linestartc = paste(" ", label2, i, " ~~ 0*", label2, i+1,  sep = "")
		if (k-i >= 2) {
			linemidc = ""
			for (j in (i+2):k) {
				linemidc = paste(linemidc, " + 0*", label2, j)
			}
		} else {
			linemidc = ""
		}
	...
	Model4 = "" # More zero covs VF 1  ~~ 0* VF 1 
	for (p in 1:k) {
		linestart4 = paste(label2, p, " ~~ 0*", label2, p)
		Model4=paste0(Model4, linestart4, " \n ")
	}
	modelCFI = paste(varStr, Model2, ModelsatF, Modelsat, Model4)
	return(modelCFI)
}
...
commonfactor <-function(covstruc, estimation="DWLS"){
	...
	# return error if model is under-identified 
	if(k == 2){
		# TODO constrain the factor loadings for the user...
		stop("There are only 2 variables in the genetic covariance matrix so the common factor model will be under identified (df = -1). 
	...
	# Create the 1 factor model of the k indicators in S_LD
