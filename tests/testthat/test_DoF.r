...
omxCheckEquals(9, m1oldWay$degreesOfFreedom - m1oldWay$saturatedDoF)

sat1 = summary(mxRefModels(m1, run=TRUE))
m1oldWay = summary(m1, SaturatedLikelihood = sat1$Minus2LogLikelihood)
summary(m1)


s2$observedStatistics # 3000
s2$degreesOfFreedom
s2$estimatedParameters

grep("lik", names(sat1), value=T, ignore.case=T)
...
DoF <- 2221 - 3 # obs stats minus estimated params n <- 39 #number of
variables that must be in data m <- 7 #number of variables used in model
satDoF <- 2221 - (n*(n-1)/2 + 2*m) DoF-satDoF #Reported incorrectly as 752 by OpenMx. Should be 32
