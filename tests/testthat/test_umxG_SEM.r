...
test_that("umxG_SEM fits simple bivariate model and subsets correctly", {
	...
	m1 = umxG_SEM(m1, S = S, V = V, estimation = "DWLS", autoRun = TRUE)
	...
	traits3 = c("T1", "T2", "T3")
	S3 = matrix(nrow = 3, ncol = 3, dimnames = list(traits3, traits3), c(
		0.50, 0.20, 0.10,
	    0.20, 0.60, 0.15,
	    0.10, 0.15, 0.70)
	)
	...
	fit_subset = umxG_SEM(model = model, S = S3, V = V3, estimation = "DWLS", autoRun = TRUE)
	
	expect_s4_class(fit_subset, "MxModel")
