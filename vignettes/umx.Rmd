...
With categorical or ordinal data, treating variables as continuous can bias standard errors and fit statistics. `umx` supports robust standard errors (sandwich estimation) for ordinal models.

Here we use `umxRAM()` to build a three-factor model (visual, textual, and speed loading on `x1:x3`, `x4:x6`, and `x7:x9` respectively) using the categorical Holzinger-Swineford dataset (`HSwls`), where the variables are coded as ordered factors:

```{r fit-ordinal, message = FALSE, warning = FALSE}
data("HSwls", package = "umx")

# Fit the three-factor model using FIML (Default type)
m3 = umxRAM("
  visual  =~ x1 + x2 + x3
  textual =~ x4 + x5 + x6
  speed   =~ x7 + x8 + x9
...
Now we compare the standard errors using standard maximum likelihood standard errors (`uncertainty = "SE"`) versus robust sandwich standard errors (`uncertainty = "RobustSE"`). We specify `refModels = FALSE` to skip the slow computation of the saturated and independence baseline models:

```{r compare-se}
# Summary with standard SEs
umxSummary(m3, std = TRUE, uncertainty = "SE", refModels = FALSE)
